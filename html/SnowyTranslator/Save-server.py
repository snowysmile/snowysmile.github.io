...
async def ichimoe_japanese_furigana(text):
    ...
    try:
        ...
        while textPos < len(text):  # is_not_japanese_character(text[textPos]):
            ...
            textPos += 1
        lprint("ichimoe_japanese_furigana() inner_html:", inner_html)
        return inner_html
    except Exception as e:
    ...
    try:
        ...
        while textPos < len(text):
            if wordId < len(words) and text[textPos: textPos + len(words[wordId])] == words[wordId]:
                ...
                inner_html += text[textPos]
                textPos += 1
        lprint("fetch_english_IPA() inner_html:", inner_html)
        return inner_html
    except Exception as e:
        print("error:", e)
    ...
    try:
        ...
        while textPos < len(text):
            if wordId < len(words) and text[textPos: textPos + len(words[wordId])] == words[wordId]:
                ...
                inner_html += text[textPos]
                textPos += 1
        lprint("fetch_english_IPA_toipa() inner_html:", inner_html)
        return inner_html
    except Exception as e:
        print("error:", e)
    ...
    try:
        ...
        while textPos < len(text):
            if pinyinId < len(pinyins) and is_chinese_kanji(text[textPos]):
                ...
                inner_html += text[textPos]
                textPos += 1
        lprint("fetch_pinyin() inner_html:", inner_html)
        return inner_html
